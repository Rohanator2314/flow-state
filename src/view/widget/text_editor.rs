...
//!
//! Vendored from iced 0.14.2 (`iced_widget::text_editor`, MIT licensed) so
//! flow-state can extend it: a decoration draw pass (character underline /
//! background / paragraph glow) and scroll readback/control for typewriter
//! centering — neither of which the stock widget exposes. Imports are remapped
//! to the public `iced` facade; the editing/layout/shaping logic is unchanged.
//! Local additions are marked `flow-state:`.

// Faithful vendor of upstream's widget: keep its full public API and upstream's
// feature gates even though flow-state uses only part of them.
