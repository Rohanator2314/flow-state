...
//! ‹/› buttons step between matches; ESC, the ✕ button, or a second CTRL+F
//! closes it and returns focus to the editor. All state lives in
//! [`App::search`](crate::app::App); this module only renders it and exposes
//! the input's focus [`Task`]. CTRL+F toggling is a global subscription in
//! `app.rs`, since the find input — not the editor — holds focus while the bar
//! is open.
